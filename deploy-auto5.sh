...
if [[ $# -lt 1 ]]; then
  echo -e "\n${RED}❌ 参数缺失！请使用格式：${NC}"
  echo "bash <(curl -Ls https://raw.githubusercontent.com/TombRaider001/deploy-node/main/deploy-batch.sh) ip:端口:用户:密码 [ip:端口:用户:密码 ...]"
  echo "例如：38.98.15.241:11553:UUKKKKJJ:WWEEE 38.98.15.341:11553:UUKKKKJJ:WWEEE"
  exit 1
fi
...
for S5_PARAM in "$@"; do
  ...
  if [[ -z "$S5_IP" || -z "$S5_PORT" || -z "$S5_USER" || -z "$S5_PASS" ]]; then
    echo -e "\n${RED}❌ 参数不完整：$S5_PARAM！请用 ip:端口:用户名:密码 的格式传递！${NC}"
    echo -e "${GRN}======================================================${NC}"
    continue
  fi
