#!/bin/bash

# 检查 bash 环境
if [ -z "$BASH_VERSION" ]; then
  echo -e "\033[31m[ERR] 请用 bash 执行本脚本！（不要用 sh 或 dash）\033[0m"
  exit 1
fi

# 日志与输出文件
LOG_DIR="/root/xui-info"
mkdir -p "$LOG_DIR"
LAST_LOG="$LOG_DIR/last_run.log"
ERR_LOG="$LOG_DIR/error.log"

# 颜色变量
RED='\033[31m'
GRN='\033[32m'
YEL='\033[33m'
CYN='\033[36m'
NC='\033[0m'

# 包管理器自动适配
if command -v apt >/dev/null 2>&1; then
  PM="apt"
  update_cmd="apt update"
  upgrade_cmd="apt upgrade -y"
  install_cmd="apt install -y"
elif command -v yum >/dev/null 2>&1; then
  PM="yum"
  update_cmd="yum -y update"
  install_cmd="yum install -y"
else
  echo -e "${RED}[ERR] 暂不支持你的 Linux 发行版！${NC}"
  exit 1
fi

echo -e "${CYN}[INFO]${NC} 正在升级系统包&检测依赖..."
DEBIAN_FRONTEND=noninteractive $update_cmd
[[ "$PM" == "apt" ]] && DEBIAN_FRONTEND=noninteractive $upgrade_cmd

for cmd in curl jq socat wget dig; do
  if ! command -v $cmd >/dev/null 2>&1; then
    echo -e "${YEL}[INFO]${NC} 缺少 $cmd，自动安装中..."
    DEBIAN_FRONTEND=noninteractive $install_cmd $cmd
  fi
done

# Cloudflare 环境变量
CF_API="${CF_API:?请先 export CF_API=你的Token}"
CF_EMAIL="${CF_EMAIL:?请先 export CF_EMAIL=你的邮箱}"

# 限制最多5条
MAX_BATCH=5
if [[ $# -gt $MAX_BATCH ]]; then
  echo -e "${YEL}[WARN]${NC} 一次最多只能申请 ${MAX_BATCH} 条（防止证书API超限），只处理前${MAX_BATCH}条，其余自动跳过"
  set -- "${@:1:$MAX_BATCH}"
fi

# 处理输入参数
if [[ $# -lt 1 ]]; then
  echo -e "\n${RED}❌ 参数缺失！请使用格式：${NC}"
  echo "bash <(curl -Ls https://raw.githubusercontent.com/TombRaider001/deploy-node/main/deploy-auto-end.sh) 协议 s5信息 ... "
  echo "协议用 1:vless 2:vmess 3:ss，s5格式 ip:端口:用户:密码"
  echo "例：1 38.106.2.221:35145:fA9aM4:Q5wA2"
  exit 1
fi

# 检查协议+S5重复输入
declare -A TASKED_S5
RESULT_OUT=""

for (( idx=1; idx<=$#; idx+=2 )); do
  proto="${!idx}"
  s5str="${!((idx+1))}"

  if [[ -z "$proto" || -z "$s5str" ]]; then
    echo -e "${RED}[ERR] 参数不全，跳过！${NC}"
    continue
  fi
  key="${proto}_${s5str}"
  if [[ -n "${TASKED_S5[$key]}" ]]; then
    echo -e "${YEL}[WARN] 重复的任务 $proto $s5str，跳过${NC}"
    continue
  fi
  TASKED_S5[$key]=1

  # 拆分 S5
  IFS=':' read -r S5_IP S5_PORT S5_USER S5_PASS <<< "$s5str"
  if [[ -z "$S5_IP" || -z "$S5_PORT" || -z "$S5_USER" || -z "$S5_PASS" ]]; then
    echo -e "${RED}[ERR] Socks5 参数不完整，跳过本条${NC}"
    continue
  fi

  BASE_DOMAIN="moneylll.top"
  TIME_STR=$(date +%Y%m%d%H%M%S)
  RAND_ID=$(( RANDOM % 10000 ))
  SUB_DOMAIN="wdch-${TIME_STR}${RAND_ID}.wdch"
  FULL_DOMAIN="${SUB_DOMAIN}.${BASE_DOMAIN}"
  INFO_FILE="$LOG_DIR/${FULL_DOMAIN}.log"
  KEY_FILE="/etc/x-ui/${FULL_DOMAIN}.key"
  CRT_FILE="/etc/x-ui/${FULL_DOMAIN}.crt"
  XUI_PORT="10000"
  VPS_IP=$(curl -s --max-time 10 ipv4.ip.sb || curl -s --max-time 10 ip.sb || hostname -I | awk '{print $1}')
  PORT=$((RANDOM%20000+20000))

  # 日志重定向（每条独立保存到总日志/错误日志）
  exec > >(tee -a "$LAST_LOG") 2>>"$ERR_LOG"

  # TCP/BBR 优化（只第一次生效）
  if ! grep -q "net.ipv4.tcp_rmem" /etc/sysctl.conf; then
cat >> /etc/sysctl.conf <<EOF
net.ipv4.tcp_rmem = 4096 87380 16777216
net.ipv4.tcp_wmem = 4096 87380 16777216
net.ipv4.tcp_max_syn_backlog = 4096
net.core.somaxconn = 4096
net.core.default_qdisc = fq
net.ipv4.tcp_congestion_control = bbr
net.ipv4.tcp_fastopen = 3
net.ipv4.tcp_window_scaling = 1
net.ipv4.tcp_slow_start_after_idle = 0
EOF
    sysctl -p
  fi

  # Cloudflare DNS 解析
  echo -e "${CYN}[INFO]${NC} 添加 Cloudflare 解析..."
  grep -q "CF_Token" /root/.bashrc || echo "export CF_Token='$CF_API'" >> /root/.bashrc
  grep -q "CF_Email" /root/.bashrc || echo "export CF_Email='$CF_EMAIL'" >> /root/.bashrc
  export CF_Token="$CF_API"
  export CF_Email="$CF_EMAIL"

  CF_ZONE=$(curl -s -X GET "https://api.cloudflare.com/client/v4/zones?name=${BASE_DOMAIN}" \
    -H "Authorization: Bearer $CF_API" -H "Content-Type: application/json" | jq -r '.result[0].id')

  if [[ -z "$CF_ZONE" || "$CF_ZONE" == "null" ]]; then
    echo -e "${RED}[ERR] Cloudflare Zone ID 获取失败，跳过本条！${NC}"
    continue
  fi

  # 添加/更新 A 记录，最多重试2次
  try=0; ok=0
  while [[ $try -lt 2 && $ok -eq 0 ]]; do
    REC_ID=$(curl -s -X GET "https://api.cloudflare.com/client/v4/zones/$CF_ZONE/dns_records?name=${FULL_DOMAIN}" \
      -H "Authorization: Bearer $CF_API" -H "Content-Type: application/json" | jq -r '.result[0].id')
    if [[ $REC_ID == "null" || -z $REC_ID ]]; then
      curl -s -X POST "https://api.cloudflare.com/client/v4/zones/$CF_ZONE/dns_records" \
        -H "Authorization: Bearer $CF_API" -H "Content-Type: application/json" \
        --data '{"type":"A","name":"'${SUB_DOMAIN}'","content":"'${VPS_IP}'","ttl":120,"proxied":false}' >/dev/null
    else
      curl -s -X PUT "https://api.cloudflare.com/client/v4/zones/$CF_ZONE/dns_records/$REC_ID" \
        -H "Authorization: Bearer $CF_API" -H "Content-Type: application/json" \
        --data '{"type":"A","name":"'${SUB_DOMAIN}'","content":"'${VPS_IP}'","ttl":120,"proxied":false}' >/dev/null
    fi
    sleep 5
    DNS_IP=$(dig +short $FULL_DOMAIN | grep -E '^[0-9.]+' | head -1)
    if [[ "$DNS_IP" == "$VPS_IP" ]]; then ok=1; break; fi
    try=$((try+1))
    echo -e "${YEL}[WARN] 第${try}次DNS生效检测失败，重试...${NC}"
  done

  if [[ $ok -eq 0 ]]; then
    echo -e "${RED}[ERR] DNS 解析未能生效，跳过本条！${NC}"
    continue
  fi

  # 证书：只给vless/vmess加证书，ss不用证书
  crt_info=""
  if [[ "$proto" == "1" || "$proto" == "2" ]]; then
    echo -e "${CYN}[INFO]${NC} 正在申请SSL证书..."
    curl https://get.acme.sh | sh
    ~/.acme.sh/acme.sh --set-default-ca --server letsencrypt
    ACME_OUT=$(
      ~/.acme.sh/acme.sh --issue --dns dns_cf -d $FULL_DOMAIN --keylength ec-256 --force 2>&1
    )
    if ! echo "$ACME_OUT" | grep -q "Your cert is in"; then
      echo -e "\n${RED}[ERR] 证书签发失败！跳过本条！${NC}\n----acme.sh输出信息如下----"
      echo -e "${RED}$ACME_OUT${NC}"
      continue
    fi
    ~/.acme.sh/acme.sh --install-cert -d $FULL_DOMAIN --ecc \
      --key-file "$KEY_FILE" \
      --fullchain-file "$CRT_FILE"
    crt_info="证书路径 (CRT)：$CRT_FILE\n证书密钥 (KEY)：$KEY_FILE"
  fi

  # 伪模拟面板入站出站（仅文本输出。你需要根据你面板实际再自动写入json或api！）
  uuid=$(cat /proc/sys/kernel/random/uuid)
  ss_pass=$(tr -dc 'A-Za-z0-9' < /dev/urandom | head -c 24)
  email="socks5-${PORT}"
  # 组装连接
  if [[ "$proto" == "1" ]]; then
    link="vless://$uuid@$FULL_DOMAIN:$PORT?type=tcp&security=tls&fp=chrome&sni=$FULL_DOMAIN#wdch-socks5-$PORT"
    protocol_text="协议：vless"
  elif [[ "$proto" == "2" ]]; then
    link="vmess://$(echo -n "{\"v\":\"2\",\"ps\":\"wdch-socks5-$PORT\",\"add\":\"$FULL_DOMAIN\",\"port\":\"$PORT\",\"id\":\"$uuid\",\"aid\":\"0\",\"net\":\"tcp\",\"type\":\"none\",\"host\":\"\",\"path\":\"\",\"tls\":\"tls\",\"sni\":\"$FULL_DOMAIN\",\"alpn\":\"h2,http/1.1\",\"fp\":\"chrome\"}" | base64 -w0)"
    protocol_text="协议：vmess"
  elif [[ "$proto" == "3" ]]; then
    link="ss://$(echo -n "aes-128-gcm:$ss_pass@$FULL_DOMAIN:$PORT" | base64 -w0)#wdch-ss5-$PORT"
    protocol_text="协议：shadowsocks"
  fi

  # 输出&保存信息
  out_msg="
------------------------------------------
${GRN}面板地址（IP直连）：http://${VPS_IP}:${XUI_PORT}/
面板地址（域名访问）：http://${FULL_DOMAIN}:${XUI_PORT}/
协议类型：$protocol_text
端口：$PORT
UUID/密码：$uuid/$ss_pass
出站 Socks5：$S5_IP:$S5_PORT:$S5_USER:$S5_PASS
${crt_info}
一键链接：$link${NC}
------------------------------------------
关键信息保存路径：$INFO_FILE
本次运行所有输出日志：$LAST_LOG
本次运行错误日志：$ERR_LOG
------------------------------------------
"
  echo -e "$out_msg" | tee "$INFO_FILE"
  RESULT_OUT+="$out_msg\n"
done

echo -e "${CYN}全部任务已完成，关键信息见 /root/xui-info 目录。${NC}"
